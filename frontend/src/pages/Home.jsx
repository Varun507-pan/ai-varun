 ...
 const handleLogout = async () => {
  ...
  useEffect(() => {
    ...
    recognition.onresult = async (event) => {
      ...
      if (
        ...
        const responseText = await getGeminiResponseWithLang(transcript, detectedLang);
    ...
    recognition.onerror = (error) => console.error("🎤 Recognition error:", error);
    recognition.start();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black flex flex-col justify-center items-center px-4 sm:px-8 py-12 relative text-center overflow-hidden">
      ...
      {/* Main */}
      <div className={`flex flex-col items-center justify-center transition-all duration-300 ${menuOpen ? "ml-64" : ""}`}>
        ...
        {userData?.assistantName && (
          <h1 className="mt-4 sm:mt-6 text-2xl sm:text-3xl font-bold text-white tracking-wide drop-shadow-md">
            I’m <span className="text-blue-400 font-semibold">{userData.assistantName}</span>
          </h1>
        )}
        ...
        <div className="mt-8 flex flex-col items-center justify-center gap-8 sm:gap-16 w-full">
          {isUserSpeaking && (
            <div className="relative flex flex-col items-center text-center">
              ...
              />
              <p className="mt-2 text-white text-base sm:text-lg font-semibold">You</p>
            </div>
          )}
          ...
          {!isUserSpeaking && aiText && (
            <div className="relative flex flex-col items-center">
              ...
              />
              <p className="mt-2 text-white text-base sm:text-lg font-semibold">{userData?.assistantName || "Assistant"}</p>
